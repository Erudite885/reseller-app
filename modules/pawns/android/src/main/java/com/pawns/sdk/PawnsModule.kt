...
class PawnsModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {

    override fun getName(): String = "PawnsModule"
    ...
    fun initialize(apiKey: String, config: ReadableMap, promise: Promise) {
        try {
            val title = config.getString("title") ?: "Bandwidth Sharing"
            val body = config.getString("body") ?: "Sharing your internet to earn rewards"
            ...
            Pawns.Builder(reactApplicationContext)
                .apiKey(apiKey)
                .serviceConfig(
                    ServiceConfig(
                        title = title,
            ...
            promise.reject("PAWNS_INIT_ERROR", e.message ?: "Failed to initialize Pawns SDK")
        }
    }
    ...
    fun startSharing(promise: Promise) {
        try {
            Pawns.getInstance().startSharing(reactApplicationContext)
            promise.resolve(true)
        } catch (e: Exception) {
    ...
    fun stopSharing(promise: Promise) {
        try {
            Pawns.getInstance().stopSharing(reactApplicationContext)
            promise.resolve(true)
        } catch (e: Exception) {
...
//         try {
//             val title = serviceConfig.getString("title") ?: "Bandwidth Sharing"
//             val body = serviceConfig.getString("body") ?: "Sharing your internet to earn rewards"
