...
function AppContent() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (user?.id && !showConsentGate && !isCheckingConsent) {
        // Small delay to ensure we're fully back
  ...
  //   // Navigate to security settings where bandwidth sharing toggle lives
  //   router.push({
  ...
  // ─── Handle Open Settings from Consent Gate ────────────────────────────
  const handleOpenSettingsFromConsent = () => {
    // setPendingConsentCheck(true); // ← Set flag to prevent immediate re-show
    // setShowConsentGate(false); // ← Close modal
