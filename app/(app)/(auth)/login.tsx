...
//       if (resellerError) {
//         console.error("[Login] Error finding reseller:", resellerError);
//       }

//       if (reseller) {
//         const { data: customer, error: customerError } = await supabase
//           .from("reseller_customers")
//           .select("auth_email, created_at")
//           .eq("email", email)
//           .eq("reseller_id", reseller.id)
//           .order("created_at", { ascending: false });

//         if (customerError) {
//           console.error("[Login] Error finding customer:", customerError);
//         }

//        if (customer && customer.length > 0) {
//          // Pick the first record that has auth_email set, newest first
//          const best = customer.find((c) => c.auth_email) ?? customer[0];
//          if (best?.auth_email) {
//            loginEmail = best.auth_email;
//            console.log("[Login] Using auth_email for login:", loginEmail);
//          }
//        }
//       }

//       // STEP 2: Login with the determined email
//       const {
//         data: { session },
//         error,
//       } = await supabase.auth.signInWithPassword({
