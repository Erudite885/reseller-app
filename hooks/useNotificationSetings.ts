...
export function useNotificationSettings() {
  ...
  async function registerForPushNotificationsAsync() {
    ...
    try {
      ...
      console.log("✅ Expo push token:", expoToken.substring(0, 30) + "...");

      // ✅ Get raw device token (FCM token for direct Firebase)
      const deviceToken = await Notifications.getDevicePushTokenAsync();
      const fcmToken = deviceToken.data;
      console.log("✅ Raw FCM token:", fcmToken.substring(0, 30) + "...");
  ...
  const checkNotificationStatus = async () => {
    ...
    try {
      ...
      const { data: reseller } = await supabase
        .from("resellers")
        .select("notifications_enabled, push_token, fcm_token")
    ...
    try {
      ...
      if (!tokens) {
        return {
          success: false,
          error:
            "Could not get push tokens. Please check your device settings.",
        };
      }
      ...
      if (reseller) {
        const { error } = await supabase
          .from("resellers")
          .update({
            push_token: tokens.expoToken, // Expo token (for compatibility)
            fcm_token: tokens.fcmToken, // Raw FCM token (for direct sending)
